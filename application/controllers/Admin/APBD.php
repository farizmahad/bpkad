...
class APBD extends CI_Controller {
    ...
    public function tren_apbd($offset=0){
        ...
        $config['base_url'] = base_url().'admin/APBD/tren_apbd';
        $config['total_rows'] = $this->Berita_model->count_tren_apbd();
        $config['per_page'] =20;
        $config['use_page_numbers'] = false;
        $config['page_query_string'] = false;
        $config['enable_query_strings'] = true;
        ...
        $data['tahun']=$this->Berita_model->select_distinc_tahun_tren();
        $data['select_trend_apbd']=$this->Berita_model->select_tren_apbd_select();

        if(!$tahun and !$keterangan) {
            $data['tren_apbd'] = $this->Berita_model->select_tren_apbd($config['per_page'], $offset, $form);
            $this->load->helper('url');
            $this->load->library('pagination');

            $data['link'] = $this->pagination->create_links();
        }else{
            $data['tren_apbd'] = $this->Berita_model->select_tren_apbd_tahun($tahun,$keterangan);
        }
        $this->load->view('admin/layouts/header');
        $this->load->view('admin/tren_apbd',$data);
        $this->load->view('admin/layouts/footer');
    }
    ...
    public function upload_trend_apbd()
    {
        //var_dump("test");die;
        $fileName = time().$_FILES['file']['name'];
        ...
        //$config['upload_path'] = './file_excel/'; //buat folder dengan nama assets di root folder
        $config['upload_path'] = FCPATH.'file_excel/tren/';
        $config['file_name'] = $fileName;
        $config['allowed_types'] = 'xls|xlsx|csv';
        $config['overwrite'] = true;
        $config['max_size'] = 0;
        ...
        for ($row = 2; $row <= $highestRow; $row++){                  //  Read a row of data into an array
            ...
            */
            ...
            //sesuaikan nama dengan nama tabel
            $insert = $this->db->insert("tren",$data);
            delete_files($media['file_path']);
            //delete_files($config['file_path'],TRUE);
        }
        redirect('admin/APBD/tren_apbd');

    }

    function edit_tren_apbd(){
        $id = $this->input->post('id');
        $uraian = $this->input->post('uraian');
        $jumlah = $this->input->post('jumlah');
        $tahun = $this->input->post('tahun');
        $this->Berita_model->update_tren_apbd($id,$uraian,$jumlah,$tahun);
        echo $this->session->set_flashdata('message','<div class="alert alert-success"> <button type="button" class="close" data-dismiss="alert">×</button>Edit data trend APBD berhasil</div>');
        redirect('admin/APBD/tren_apbd');
    }

    function delete_tren_apbd($id_tren)
    {


        $this->Berita_model->delete_tren_apbd($id_tren);
        echo $this->session->set_flashdata('message', '<div class="alert alert-info"><B>Trend APBD berhasil dihapus</b></div>');
        redirect('admin/APBD/tren_apbd');
    }

    function edit_visualisasi_apbd(){
        $id = $this->input->post('id');
        $uraian = $this->input->post('uraian');
        $jumlah = $this->input->post('jumlah');
        $tahun = $this->input->post('tahun');
        ...
        $this->Berita_model->update_visualisasi_apbd($id,$uraian,$jumlah,$tahun,$status);
        echo $this->session->set_flashdata('message','<div class="alert alert-success"> <button type="button" class="close" data-dismiss="alert">×</button>Edit data visualisasi APBD berhasil</div>');
        redirect('admin/APBD');
    }

    function delete_visualisasi_apbd($id_tren)
    {


        $this->Berita_model->delete_visualisasi_apbd($id_tren);
        echo $this->session->set_flashdata('message', '<div class="alert alert-info"><B>Visualiasi APBD berhasil dihapus</b></div>');
        redirect('admin/APBD');
    }
