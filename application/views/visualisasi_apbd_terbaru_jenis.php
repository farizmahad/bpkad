...
<div id="result">
    ...
    <?php 
        ...
        $tahun=$peri->tahun;
        $select_sum_all=$this->Berita_model->select_sum_all($tahun,$jenis);
            foreach($select_sum_all as $all){
                $jumlah_keseluruhan=$all->jumlah;
            }
        $select_sum_pendapatan=$this->Berita_model->select_sum_pendapatan($tahun,$jenis);
         foreach($select_sum_pendapatan as $p4){
                $jumlah_pendapatan=$p4->jumlah;
        }

         $select_sum_belanja=$this->Berita_model->select_sum_belanja($tahun,$jenis);
         foreach($select_sum_belanja as $b5){
                $jumlah_belanja=$b5->jumlah;
        }

        $select_sum_pembiayaan=$this->Berita_model->select_sum_pembiayaan($tahun,$jenis);
...
Highcharts.chart('container<?php echo $nos++; ?>', {
    ...
    title: {
        text: 'Visualisasi APBD Kota Cimahi <?php echo $jenis; ?> tahun <?php echo $peri->tahun; ?>'
    },
    tooltip: {
